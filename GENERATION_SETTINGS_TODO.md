...
- Changed: secondary-anchor selection now targets compact group sizing around ~3 systems per anchor cluster when possible.
- Changed: anchor-cluster growth now enforces a hard max of 5 systems per anchor cluster.
...
- Changed: deep-space POI generation now targets a density-aware count based on grid size and generation density settings.
